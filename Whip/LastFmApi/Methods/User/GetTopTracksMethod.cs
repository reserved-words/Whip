...
{
    internal class GetTopTracksMethod : ApiMethodBase<ICollection<UserTrackInfo>>
    {
        public GetTopTracksMethod(ApiClient client, string username, TimePeriod timePeriod, int limit)
            : base(client, "user.gettoptracks")
        {
        ...
        public override ICollection<UserTrackInfo> ParseResult(XElement xml)
        {
            var resultsXml = xml.Element("toptracks");
            var list = new List<UserTrackInfo>();

            foreach (var result in resultsXml.Elements("track"))
            {
                list.Add(new UserTrackInfo
                {
